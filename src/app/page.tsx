...
export default function LoginPage() {
        ...
        <Card className="w-full max-w-lg text-center p-8 border-destructive">
            <CardHeader>
                ...
                <CardDescription className="text-destructive/90">
                    The application cannot connect to Firebase because your credentials are not set up correctly.
                </CardDescription>
            </CardHeader>
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      activateProTrial(); // Activate trial on signup
      toast({ title: 'Account Created!', description: "Welcome to Idea Saver!" });
      router.push('/record');
    } catch (error) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (additionalInfo?.isNewUser) {
        activateProTrial(); // Activate trial for new Google users
      }
      router.push('/record');
    } catch (error) {
      handleAuthError(error);
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
               ...
               <Button onClick={handleSignUp} className="w-full" disabled={isLoading}>
                {isLoading ? <Loader2 className="animate-spin" /> : 'Sign Up & Start Free Trial'}
              </Button>
            </CardContent>
            <CardFooter className="flex-col items-center justify-center text-center text-sm gap-2 pt-4">
